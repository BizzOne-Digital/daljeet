...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="site-section bg-grey py-16 sm:py-20"><div className="site-container"><SectionHeading eyebrow="Values" title="Mission & Philosophy" /><div className="mt-8 grid gap-6 lg:grid-cols-2"><div className="min-w-0 grid gap-4 md:grid-cols-2">{whyChooseUs.map((item) => <div key={item.title} className="min-w-0 rounded-2xl bg-white p-6 shadow-sm"><h3 className="font-display text-lg font-bold">{item.title}</h3><p className="mt-2 text-sm text-slate">{item.description}</p></div>)}</div><Image src={siteImages.sections.trust} alt="Professional cleaning supplies and equipment" width={600} height={700} className="responsive-img min-h-[240px] rounded-2xl object-cover shadow-lg sm:min-h-[320px]" /></div></div></section>
      <section className="site-section py-16 sm:py-20"><div className="site-container"><SectionHeading eyebrow="Process" title="Our Quality Process" /><div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{cleaningProcess.map((step) => <div key={step.step} className="min-w-0 rounded-2xl border border-brand/10 p-5"><p className="font-display text-2xl font-bold text-brand">{step.step}</p><h3 className="mt-2 font-bold">{step.title}</h3><p className="mt-2 text-sm text-slate">{step.description}</p></div>)}</div><Image src={siteImages.sections.process} alt="Process" width={1200} height={420} className="responsive-img mt-8 aspect-[4/3] rounded-2xl object-cover shadow-lg sm:aspect-[21/9]" /></div></section>
      <section className="site-section bg-grey py-16 sm:py-20"><div className="site-container"><SectionHeading eyebrow="Community" title="Proudly Serving the GTA" description="Residential homes, condos, commercial spaces, and property-management clients across the Greater Toronto Area." /><Image src={siteImages.sections.gta} alt="GTA community" width={1200} height={420} className="responsive-img mt-8 aspect-[4/3] rounded-2xl object-cover shadow-lg sm:aspect-[21/9]" /></div></section>
