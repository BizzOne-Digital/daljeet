...
export default function PricingPage() {
  return (
    <>
      ...
      <section className="site-section py-16 sm:py-20"><div className="site-container max-w-4xl"><div className="rounded-3xl border border-brand/10 bg-white p-6 shadow-sm sm:p-8"><h2 className="font-display text-xl font-bold sm:text-2xl">What may influence your quote</h2><ul className="mt-6 space-y-3 text-slate">{quoteFactors.map((f) => <li key={f}>• {f}</li>)}</ul><div className="mt-8 grid gap-4 sm:grid-cols-2"><Image src={siteImages.sections.livingRoom} alt="Residential cleaning quote" width={450} height={300} className="responsive-img rounded-2xl border border-brand/10 object-cover" /><Image src={siteImages.services.commercial} alt="Commercial cleaning quote" width={450} height={300} className="responsive-img rounded-2xl border border-brand/10 object-cover" /></div></div></div></section>
      <section className="site-section bg-grey py-16 sm:py-20"><div className="site-container grid gap-8 lg:grid-cols-2 lg:items-center"><Image src={siteImages.services.epoxy} alt="Epoxy flooring service" width={600} height={420} className="responsive-img rounded-2xl border border-brand/10 object-cover" /><div className="min-w-0 rounded-3xl bg-gradient-to-br from-brand to-cyan p-6 text-white sm:p-8"><h2 className="break-words font-display text-2xl font-extrabold sm:text-3xl">{siteConfig.offer.headline}</h2><p className="mt-3">{siteConfig.offer.description}</p><p className="mt-2 text-xs text-white/75">{siteConfig.offer.termsNote}</p><Link href="/contact" className="mt-6 inline-flex w-full justify-center rounded-full bg-white px-6 py-3 text-sm font-bold text-brand sm:w-auto">Request Quote</Link></div></div></section>
